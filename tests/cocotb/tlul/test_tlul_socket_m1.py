...
async def test_arbitration_and_ordering(dut):
    ...
    try:
        ...
        responder_tasks = [cocotb.start_soon(device_responder(j)) for j in range(env.N)]
        ...
        traffic_tasks = [cocotb.start_soon(send_master_traffic(i)) for i in range(M)]
        await Combine(*traffic_tasks)

        # 3. Wait for all responses to finish
        total_txns = M * num_txns_per_master
        cycles = 0
        while len(env.scoreboard.global_response_order) < total_txns and cycles < 200:
            await ClockCycles(dut.clock, 1)
            cycles += 1
            
        assert len(env.scoreboard.global_response_order) == total_txns, "Test timed out waiting for responses"

        # --- VERIFICATION ---
        # Check 1: Fixed-priority arbitration (Master 0 wins first request)
        first_req_master = get_master_from_source(env.scoreboard.slave_received_order[0])
...
async def test_directed_backpressure(dut):
    ...
    try:
        ...
        await device.device_respond(opcode=0, param=0, size=req_seen["size"], source=req_seen["source"])
        ...
        async def respond_0():
            await device.device_respond(opcode=0, param=0, size=req_seen_0["size"], source=req_seen_0["source"])
        ...
        async def respond_1():
            await device.device_respond(opcode=0, param=0, size=req_seen_1["size"], source=req_seen_1["source"])
        ...
        assert m1.host_d_fifo.qsize() == 0, "Master 1 received response out-of-order"
...
async def test_random_backpressure(dut):
    ...
    try:
        ...
        total_txns = M * num_txns_per_master
        cycles = 0
        while len(env.scoreboard.global_response_order) < total_txns and cycles < 500:
            await ClockCycles(dut.clock, 1)
            cycles += 1
            
        assert len(env.scoreboard.global_response_order) == total_txns, f"Test timed out. Completed {len(env.scoreboard.global_response_order)}/{total_txns} txns"
